...
(function () {
  ...
  function syncProjectCount() {
    const total = document.querySelectorAll('.projeto-card').length;
    document.querySelectorAll('[data-project-count]').forEach((el) => {
      el.dataset.count = String(total);
      // Mantém o valor correto mesmo antes da animação de contagem iniciar.
      if (!el.classList.contains('counted')) el.textContent = '0';
    });
    document.querySelectorAll('[data-project-count-text]').forEach((el) => {
